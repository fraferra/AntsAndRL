...
class SarsaBot():
    ...
    def returnState(self,S, ants):
        ...
        if S in self.other_ants_positions:
            STATE = "MY_ANT"
            PROXIMITY = 0.1
            #POSITION_RELATIVE_TO_ANT = self.getDirections()
    ...
    def returnValueState(self, S, ants):
        STATE,STATE_TOGGLE,PROXIMITY = self.returnState(S, ants)
        value = 0.0
    ...
    def selectA(self, S, ants):
        ...
        for direction in directions:
            ...
            STATE_STATE_TOGGLE = self.returnState(S_prime, ants)
            ...
            self.Q_sa[STATE_STATE_TOGGLE] = 0.5*(self.Q_sa.get(STATE_STATE_TOGGLE, 0.0) +  self.returnValueState(S, ants))
            value = self.Q_sa.get(STATE_STATE_TOGGLE, 0.0)#self.returnValueState(S_next, ants) + (1.0 + 1/self.loop)/self.S.get(S_next, 0.1)

            #self.Q_sa[(direction,STATE_STATE_TOGGLE)] = 0.5*(self.Q_sa.get((direction,STATE_STATE_TOGGLE), 0.0) +  self.returnValueState(S, ants))
            #value = self.Q_sa.get((direction,STATE_STATE_TOGGLE), 0.0)#self.returnValueState(S_next, ants) + (1.0 + 1/self.loop)/self.S.get(S_next, 0.1)
    ...
    def updateDelta(self, R, A, A_prime, S, S_prime, ants):
        STATE_STATE_TOGGLE = self.returnState(S, ants)
        STATE_STATE_TOGGLE_prime = self.returnState(S_prime, ants)
