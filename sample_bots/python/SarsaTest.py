...
class SarsaBot():
    ...
    # def treeStateSearch(self, S, ants):
    #     # value = 0.0
    #     # for i in range(10):
    #     #     value += 0.9*self.returnValueStateBasedOnObstaclesAround(S, ants)
    ...
    def selectAbasedonQ(self, S, ants):
        ...
        for direction in directions:
            ...
            value = self.Q_sa.get((direction, next_S), 0.0) + self.returnValueState(next_S, ants)+ 0.8*self.returnValueStateBasedOnObstaclesAround(next_S, ants)
    ...
    def returnValueStateBasedOnObstaclesAround(self, S, ants):
        directions = ["n", "e","s","w"]
        value = 0.0
        for direction in directions:
            next_S = self.getNextState(S, direction, ants)
            value += self.returnValueState(next_S, ants)
        return value
    ...
    def do_turn(self, ants):
        ...
        for a_row, a_col in ants.my_ants():
            ...
            self.S[S] = 0.85*self.S.get(S, 0.0) + 1.0
