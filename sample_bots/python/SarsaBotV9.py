...
class SarsaBot():
    ...
    def returnState(self,S, S_next, current_direction, ants):
        ...
        if S_next in self.other_ants_positions:
            STATE = "MY_ANT"
            PROXIMITY = 0.1
            POSITION_RELATIVE_TO_ANT = self.isBlocked( S, S_next, ants)#self.sameDirection(current_direction, self.getDirections(S, S_next, ants))
            ...
            if S_next == hill_location:
                STATE = "ENEMY_HILL"
                PROXIMITY = 0.1
                POSITION_RELATIVE_TO_ANT = self.isBlocked( S, hill_location, ants)#self.sameDirection(current_direction, self.getDirections(S, hill_location, ants))
                return (STATE, STATE_TOGGLE, PROXIMITY, POSITION_RELATIVE_TO_ANT)
            else:
                STATE = "ENEMY_HILL"
            ...
            if S_next == food_location:
                STATE = "FOOD"
                PROXIMITY = 0.1
                POSITION_RELATIVE_TO_ANT = self.isBlocked( S, food_location, ants)#self.sameDirection(current_direction, self.getDirections(S, food_location, ants))
                return (STATE, STATE_TOGGLE, PROXIMITY, POSITION_RELATIVE_TO_ANT)
            else:
                STATE = "FOOD"
            ...
            if S_next == enemy_location:
                STATE = "ENEMY_ANT"
                PROXIMITY = 0.1
                POSITION_RELATIVE_TO_ANT = self.isBlocked( S, enemy_location, ants) #self.sameDirection(current_direction, self.getDirections(S, enemy_location, ants))
                return (STATE, STATE_TOGGLE, PROXIMITY, POSITION_RELATIVE_TO_ANT)
            else:
                STATE = "ENEMY_ANT"
            ...
            STATE = "NOTHING"
            PROXIMITY = 1.0
            POSITION_RELATIVE_TO_ANT = True#self.getDirections(S, S_next, ants)
            return (STATE, STATE_TOGGLE, PROXIMITY, POSITION_RELATIVE_TO_ANT)
        return (STATE, STATE_TOGGLE, PROXIMITY)     

    #def isBlocked(self, )

    def returnValueState2(self, S, S_next, A, ants):
        proximity_coefficient = 0.1

        STATE,STATE_TOGGLE,PROXIMITY,POSITION_RELATIVE_TO_ANT = self.returnState(S, S_next,A, ants)
        value = 0.0
        if POSITION_RELATIVE_TO_ANT == True and STATE == "MY_ANT":
            value -= 10.0
        if POSITION_RELATIVE_TO_ANT == True and STATE == "WALL":
            value -= 3.0
        if POSITION_RELATIVE_TO_ANT == True and STATE != "MY_ANT" and STATE != "WALL":
            value += 2.0
        if POSITION_RELATIVE_TO_ANT == False and STATE != "MY_ANT" and STATE != "WALL":
            value -= 5.0
        ...
        if STATE_TOGGLE == "SEEN":
            value -= 0.1
            #value -= (0.0 + 1.0/self.loop + math.log(self.seen_states.get(S, 1.0)))
        if STATE == "MY_ANT":
            value -= (100.0 + proximity_coefficient*PROXIMITY)
        if STATE == "WALL":
            value -= (6.0 + proximity_coefficient*PROXIMITY) 
        if STATE == "FOOD":
            value += (5.0 - 0.5*proximity_coefficient*PROXIMITY) 
        if STATE == "ENEMY_HILL":
            value += (7.0 - proximity_coefficient*PROXIMITY) 
        if STATE == "ENEMY_ANT":
            value += (2.0 - proximity_coefficient*PROXIMITY) 
        if STATE == "NOTHING":
            value += (1.0 - proximity_coefficient*PROXIMITY) 
        return value
    ...
    def returnValueState(self, S, S_next, A, ants):
        ...
        if STATE_TOGGLE == "UNSEEN":
            ...
            #value += (2.0 + 1.0/self.loop)
        if STATE_TOGGLE == "SEEN":
            value -= self.seen_states.get(S, 1.0)
        if STATE == "FOOD" and PROXIMITY == 0.1:
            value += 1.0
        if STATE == "ENEMY_HILL" and PROXIMITY == 0.1:
            value += 2.0
        if STATE == "MY_ANT" and PROXIMITY == 0.1:
            value -= 2.0
        return value
    ...
    def save_obj(self,obj):
        with open('learningV9.pkl', 'wb') as f:
             pickle.dump(obj, f, pickle.HIGHEST_PROTOCOL)
        sys.stderr.write("SAVED!!!!! \n\n\n")

    def load_obj(self):
        try:
            with open('learningV9.pkl', 'rb') as f:
